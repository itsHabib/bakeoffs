...
if git -C "$repo" rev-parse --git-dir >/dev/null 2>&1; then
  added="$(git -C "$repo" log --diff-filter=A --format=%h -- adapters/dir.go | tail -n 1)"
  [ -n "$added" ] && git -C "$repo" show --stat --format='commit %h %s' "$added" | grep -v '^$'
fi
...
(cd "$repo" && run "grep -rl --include='*.go' --exclude='*_test.go' 'hack-workbench-hcl/adapters\"' internal/ || echo none")
