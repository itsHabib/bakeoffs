// Command interject runs a listener that is allowed to speak only a few times
// an hour, and has to decide — while you are still talking — whether whatever
// it has to say is worth one of them.
package main
...
import (
	...
	"github.com/itsHabib/interject/internal/session"
...
func main() {
	if err := run(); err != nil {
		fmt.Fprintln(os.Stderr, "interject:", err)
		os.Exit(1)
	}
}
...
func run() error {
	...
	addr := flag.String("addr", "127.0.0.1:8722", "listen address")
	...
	flag.IntVar(&cfg.Total, "budget", cfg.Total, "interruptions allowed per window")
	flag.DurationVar(&cfg.Window, "window", cfg.Window, "budget window")
	flag.DurationVar(&cfg.Cooldown, "cooldown", cfg.Cooldown, "minimum gap between interruptions")
