...
//
// The load-bearing trick is that classification runs while the person is still
// talking, not in the gap after they stop. A local model needs a second or two
// to form an opinion, which is far too slow to answer "speak now?" at the
// moment a pause opens. So the opinion is always already there, and the pause
// only reads it. Deciding continuously is what makes deciding instantly possible.
package session
...
import (
	...
	"github.com/itsHabib/interject/internal/budget"
	"github.com/itsHabib/interject/internal/listen"
	"github.com/itsHabib/interject/internal/stream"
)
...
// Session is one person, one budget, one running conversation.
type Session struct {
	log    *slog.Logger
...
func New(log *slog.Logger, scorer *listen.Scorer, hub *stream.Hub, cfg budget.Config) *Session {
	return &Session{
		...
		ledger:  budget.New(cfg, time.Now()),
	}
}
...
// Hear takes one chunk of recognised speech. It returns immediately: the
// classifier runs behind it, and drops requests that arrive while it is busy,
// so a fast talker cannot queue up a backlog of stale opinions.
...
func (s *Session) classify(ctx context.Context) {
	...
	s.apply(reading, depth)
}

// apply folds one reading into the budget and takes the floor if it is worth it.
func (s *Session) apply(reading listen.Reading, depth int) {
	kind := budget.ParseKind(reading.Kind)
	conf := budget.ParseConfidence(reading.Conf)
	raw := budget.Raw(kind, conf, depth)
	if kind != budget.KindNone && reading.Line == "" {
		raw = 0 // an opinion it cannot phrase is not an opinion
	}
...
// Verdict folds the listener's judgement of one interruption into the bar.
...
// Reset starts a fresh window, keeping nothing.
func (s *Session) Reset() {
	s.mu.Lock()
	s.ledger = budget.New(s.cfg, time.Now())
	s.transcript = nil
	s.spoken = nil
	s.pending = nil
	s.lastRead = listen.Reading{}
	s.mu.Unlock()
